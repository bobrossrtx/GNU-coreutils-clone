...
/* Rewritten by Owen Boreham.  */

// #include <config.h> Commented out for time being
...
size_t buffer_lcm(size_t a, size_t b, size_t lcm_max) {
  ...
  if (!a) size = b ? b : 8 * 1024;
  else {
    if (b) {
      // Return lcm (A, B) if it is in range; otherwise, fallback on A.

      size_t lcm, m, n, q, r;

      // N = gcd(A, B), if in range.
      q = a / n;
      lcm = q * b;
      if (lcm <= lcm_max && lcm / b == q) return lcm;
    }
    size = a;
  }
